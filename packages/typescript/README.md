# @jokelboard/typescript

TypeScript client for the [Jokelboard](https://jokelboard.com) API. Strict types, ESM + CJS dual build, auto-retry on rate limits.

## Install

```bash
npm install @jokelboard/typescript
```
...
```ts
import { JokelboardError, RateLimitError, RevisionConflictError } from '@jokelboard/typescript';
...
See the [full API reference](../../docs/api-reference.md) and [examples](../../docs/examples/basic.ts).
